...
def main():
    ...
    autoencoder.evaluate_model(test_dataloader, criterion, device, save_results=True, results_dir=os.path.join(model_save_dir, "inference_results"), results_filename=f"{identifier}_results.h5", zero_masking = True, zero_masking_model=classifier)
    results_file = os.path.join(model_save_dir, "inference_results", f"{identifier}_results.txt")
    with open(results_file, 'w') as f:
        f.write("Model Training Results\n")
        f.write("======================\n")
        f.write(f"Data Path: {datapath}\n")
        f.write(f"Model Save Directory: {model_save_dir}/inference_results\n")
